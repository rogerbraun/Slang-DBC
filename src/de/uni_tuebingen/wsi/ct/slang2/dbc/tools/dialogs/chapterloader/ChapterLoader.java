 ...
 * @author Volker Klöbb
 */
public class ChapterLoader extends JComponent implements ListSelectionListener, ActionListener
{
    ...
    */
	...
	private JDialog createDialog( JFrame parent )
	{
		...
		JScrollPane sp1 = new JScrollPane(bookList, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
		...
		JScrollPane sp2 = new JScrollPane(chapterList, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
				JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
		...
		deleteChapterButton.addActionListener(this);
		deleteChapterButton.setEnabled(false);
		c.gridx = 0;
		c.gridy = 3;
		c.gridheight = 1;
		p.add(deleteChapterButton, c);
	...
	public void valueChanged( ListSelectionEvent e )
	{
		...
		else if (e.getSource() == chapterList)
		{
			...
			if (chapter != null)
			{
				...
				deleteChapterButton.setEnabled(true);
				deleteBookButton.setEnabled(true);
			}
		}
	}
	...
	public void actionPerformed( ActionEvent e )
	{
		...
		else if (e.getSource() == copyButton)
		{
			...
			try
			{
				// TODO: Das Laden ist überflüssig! Das Kopieren sollte der
				// Server Seite überlassen werden.
		...
		else if (e.getSource() == deleteBookButton)
		{
			...
			try
			{
				int userDecision = JOptionPane.showConfirmDialog(this, "Are you sure you want to delete this book?",
						"Warning", JOptionPane.YES_NO_OPTION);
				if (userDecision == 0)
				{
					dbc = new DBC(server);
					dbc.deleteBook(bookID);
					chapterList.clearSelection();
		...
		else if (e.getSource() == deleteChapterButton)
		{
			DBC dbc = null;
			try
			{
				int userDecision = JOptionPane.showConfirmDialog(this, "Are you sure you want to delete this chapter?",
						"Warning", JOptionPane.YES_NO_OPTION);
				if (userDecision == 0)
				{
					dbc = new DBC(server);
